...
public class MainController implements Initializable {
    ...
    private void configurarColumnas() {
        ...
        colDisponible.setCellValueFactory(c -> new SimpleBooleanProperty(c.getValue().getDisponible()));
    ...
    public void cargarDatos() {
        datos.setAll(servicio.obetenerTodos());
        actualizarEstado();
    }

    private void actualizarEstado() {
        int total = datos.size();
        long disponibles = datos.stream().filter(Libro::getDisponible).count();
        lblEstado.setText("Total: " + total + " libro(s)  |  Disponibles: " + disponibles);
    }

    private void configurarBotones() {
        // Editar, Eliminar, Detalle solo activos cuando hay selección
    ...
    private void abrirFormulario(Libro libro) {
        try {
            ...
            scene.getStylesheets().add(getClass().getResource("/com/biblioteca/views/styles.css").toExternalForm());
